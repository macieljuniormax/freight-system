...
describe 'Usuário faz uma consulta' do
  ...
  it 'e volta para a tela de consultas' do
    # Arrange
    first_carrier = Carrier.create!(corporate_name: 'Transportes Next LTDA', 
                                    brand_name: 'Next Transporte', 
                                    registration_number: '12.123.123/0001-01', 
    ...
    fill_in 'Distância',	with: 300
    click_on 'Buscar'
    click_on 'Realizar Nova Consulta'

    # Assert
    expect(page).to have_content('0.1m') 
    expect(page).to have_content('0.2m') 
    expect(page).to have_content('0.3m') 
    expect(page).to have_content('20.0Kg') 
    expect(page).to have_content('300Km')
  end
end